...
$loadQuestionsForPackage = function(array $packageRow) use ($db) {
    $questions = [];
    $infoMessage = '';
    $adhdQuestions = [];

    if (!empty($packageRow['includes_mmpi']) && !empty($packageRow['includes_adhd'])) {
        $testType = 'both';
    } elseif (!empty($packageRow['includes_mmpi'])) {
        $testType = 'mmpi';
    } elseif (!empty($packageRow['includes_adhd'])) {
        $testType = 'adhd';
    } else {
        $testType = '';
    }
...
$isFreeAccessRequest = isset($_GET['free_access']) && (string)$_GET['free_access'] === '1';
$clientHintTimeRemaining = ($urlTimeRemaining !== null && is_numeric($urlTimeRemaining))
    ? max(0, intval($urlTimeRemaining))
    : null;

$testMode = $isFreeAccessRequest && isFreeTestEnabledForUser($userId);
...
try {
    ...
    if (isset($_GET['ajax_get_question']) && $_GET['ajax_get_question'] == '1') {
        ...
        $ajaxQuestionSet = $loadQuestionsForPackage($ajxSession);
        $ajxQuestions = $ajaxQuestionSet['questions'];
    ...
    $questionSet = $loadQuestionsForPackage($sessionData);
    $testType = $questionSet['test_type'];
    $questions = $questionSet['questions'];
    if (!empty($questionSet['info_message'])) {
        $infoMessage = $questionSet['info_message'];
    ...
    if (!empty($sessionData['id']) && (
        intval($sessionData['current_page'] ?? 0) !== $resumeQuestion
        || intval($sessionData['total_pages'] ?? 0) !== $totalQuestions
    )) {
        try {
            $stmt = $db->prepare("
                ...
                AND status IN ('not_started', 'in_progress')
            ");
            $stmt->execute([$resumeQuestion, $totalQuestions, $sessionData['id']]);
            $sessionData['current_page'] = $resumeQuestion;
            $sessionData['total_pages'] = $totalQuestions;
        } catch (Exception $e) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        switch ($action) {
            ...
            case 'finish_test':
                ...
                $totalDurationSeconds = max(0, (int)($sessionData['duration_minutes'] ?? 0) * 60);
                $clientTimeUsed = isset($_POST['time_used']) ? max(0, (int)$_POST['time_used']) : null;
                $finalTimeRemaining = (int)($latestSession['time_remaining'] ?? 0);
                if ($clientTimeUsed !== null && $totalDurationSeconds > 0) {
                    $clientTimeUsed = min($clientTimeUsed, $totalDurationSeconds);
                    $clientReportedRemaining = max(0, $totalDurationSeconds - $clientTimeUsed);
                    $finalTimeRemaining = min($finalTimeRemaining, $clientReportedRemaining);
                }
    ...
    <style>
        ...
        * {
        ...
        @media (max-width: 768px) {
            ...
            .mobile-sidebar-actions .sidebar-title {
                margin-bottom: 0.75rem;
            }
            ...
            .mobile-sidebar-actions .action-btn {
                min-height: 44px;
                margin-bottom: 0;
            ...
            .mobile-sidebar-actions .action-btn i {
                margin-right: 0;
            }
        ...
        <div class="test-header">
            <div class="test-header-content">
                <div class="test-title-section">
                    ...
                    <button class="theme-toggle-btn" type="button" id="themeToggle" onclick="toggleTheme()" aria-label="Ganti tema">
        ...
        <div class="test-main">
            ...
            <div class="test-sidebar">
                ...
                <div class="mobile-sidebar-actions">
                    ...
                    <div class="mobile-sidebar-actions-grid">
                        <a href="dashboard.php" class="action-btn" onclick="closeMobileTestMenu(); return goHomeSafely(event);">
        ...
        <div class="test-modal" id="finishModal">
            <div class="modal-content">
                ...
                <div style="display: flex; gap: 1rem; justify-content: flex-end;">
                    ...
                    <form method="POST" action="" style="display: inline;" id="finishForm">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (finishForm) {
            finishForm.addEventListener('submit', async function(e) {
                if (finishForm.dataset.submitting === '1') {
                    return;
                }
                ...
                try {
                    if (hasUnsavedChanges) {
                        const saveResult = await saveCurrentAnswer();
                        if (!saveResult || !saveResult.success) {
                            throw new Error('Jawaban terakhir belum tersimpan.');
                        }
                    }
                    ...
                    alert(err.message || 'Gagal menyimpan jawaban terakhir. Silakan coba lagi.');
                }
            });
        }
    });
